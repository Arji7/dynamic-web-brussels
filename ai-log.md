...
Een specifiek CSS-probleem heb ik met AI opgelost. In de kaarten wilde ik dat de footer (met de website-link en de favorietenknop) altijd onderaan de kaart staat, ook als de ene kaart meer tekst bevat dan de andere.
...
Voor een klein onderdeel gebruikte ik AI: de `getIcon` functie, die een passend emoji-icoon teruggeeft op basis van het type locatie (museum, monument, bar, ...). Ik kon dit met losse `if`-statements wel zelf, maar wilde een nettere, efficientere structuur.
...
**AI gebruikt:** Deels — alleen de `getIcon` functie (efficiente structuur gevraagd). De rest (kaarten, switchView, reset, teller, localStorage) zelf gedaan.
...
**AI gebruikt:** Ja — de bug werd met hulp van AI geidentificeerd en opgelost.
...
| getIcon functie | Ja | Efficiente structuur gevraagd |
| Bug fix sortering | Ja | Bug geidentificeerd met hulp van AI |
